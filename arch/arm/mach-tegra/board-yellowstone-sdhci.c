/*
 * arch/arm/mach-tegra/board-yellowstone-sdhci.c
 *
...
#define YELLOWSTONE_WLAN_RST	TEGRA_GPIO_PCC5
#define YELLOWSTONE_WLAN_PWR	TEGRA_GPIO_PX7
#define YELLOWSTONE_WLAN_WOW	TEGRA_GPIO_PU5
...
#define YELLOWSTONE_SD_CD	TEGRA_GPIO_PV2
#define YELLOWSTONE_SD_WP	TEGRA_GPIO_PQ4
#define FUSE_SOC_SPEEDO_0	0x134

static void (*wifi_status_cb)(int card_present, void *dev_id);
static void *wifi_status_cb_devid;
static int yellowstone_wifi_status_register(void (*callback)(int , void *), void *);

static int yellowstone_wifi_reset(int on);
static int yellowstone_wifi_power(int on);
static int yellowstone_wifi_set_carddetect(int val);
static int yellowstone_wifi_get_mac_addr(unsigned char *buf);

static struct wifi_platform_data yellowstone_wifi_control = {
	.set_power	= yellowstone_wifi_power,
	.set_reset	= yellowstone_wifi_reset,
	.set_carddetect	= yellowstone_wifi_set_carddetect,
	.get_mac_addr	= yellowstone_wifi_get_mac_addr,
#if defined (CONFIG_BCMDHD_EDP_SUPPORT)
...
static struct platform_device yellowstone_wifi_device = {
	.name		= "bcm4329_wlan",
	.id		= 1,
	.num_resources	= 1,
	.resource	= wifi_resource,
	.dev		= {
		.platform_data = &yellowstone_wifi_control,
	},
};
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data0 = {
	.mmc_data = {
		.register_status_notify	= yellowstone_wifi_status_register,
#ifdef CONFIG_MMC_EMBEDDED_SDIO
...
static int yellowstone_wifi_status_register(
...
static int yellowstone_wifi_power(int on)
{
	...
	gpio_set_value(YELLOWSTONE_WLAN_PWR, on);
	gpio_set_value(YELLOWSTONE_WLAN_RST, on);
	mdelay(100);
...
static int _yellowstone_wifi_get_mac_addr_nct(unsigned char *buf)
{
	int ret = -ENODATA;
...
#define YELLOWSTONE_WIFI_MAC_ADDR_FILE	"/mnt/persist/wifi/wifi_mac.txt"
static int _yellowstone_wifi_get_mac_addr_file(unsigned char *buf)
{
	struct file *fp;
	int rdlen;
	char str[32];
	...
	fp = filp_open(YELLOWSTONE_WIFI_MAC_ADDR_FILE, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		pr_err("%s: cannot open %s\n",
			__func__, YELLOWSTONE_WIFI_MAC_ADDR_FILE);
		return -ENOENT;
	}
...
static int yellowstone_wifi_get_mac_addr(unsigned char *buf)
{
	...
	if (_yellowstone_wifi_get_mac_addr_nct(buf))
		return _yellowstone_wifi_get_mac_addr_file(buf);

	return 0;
}

static int __init yellowstone_wifi_init(void)
{
	int rc;

	rc = gpio_request(YELLOWSTONE_WLAN_PWR, "wlan_power");
	if (rc)
		pr_err("WLAN_PWR gpio request failed:%d\n", rc);
	rc = gpio_request(YELLOWSTONE_WLAN_RST, "wlan_rst");
	if (rc)
		pr_err("WLAN_RST gpio request failed:%d\n", rc);
	rc = gpio_request(YELLOWSTONE_WLAN_WOW, "bcmsdh_sdmmc");
	if (rc)
		pr_err("WLAN_WOW gpio request failed:%d\n", rc);

	rc = gpio_direction_output(YELLOWSTONE_WLAN_PWR, 0);
	if (rc)
		pr_err("WLAN_PWR gpio direction configuration failed:%d\n", rc);
	rc = gpio_direction_output(YELLOWSTONE_WLAN_RST, 0);
	if (rc)
		pr_err("WLAN_RST gpio direction configuration failed:%d\n", rc);

	rc = gpio_direction_input(YELLOWSTONE_WLAN_WOW);
	if (rc)
		pr_err("WLAN_WOW gpio direction configuration failed:%d\n", rc);

	wifi_resource[0].start = wifi_resource[0].end =
		gpio_to_irq(YELLOWSTONE_WLAN_WOW);

	platform_device_register(&yellowstone_wifi_device);
...
static int __init yellowstone_wifi_prepower(void)
{
	yellowstone_wifi_power(1);

	return 0;
}

subsys_initcall_sync(yellowstone_wifi_prepower);
#endif

int __init yellowstone_sdhci_init(void)
{
	int nominal_core_mv;
